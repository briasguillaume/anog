...
class InteractBDD(Static):
	...
	def removeFighter(username, pirate):
		...
		request = "DELETE FROM pirate WHERE username like '"+username+"' and name like '"+pirate.name+"' and fruit like '"+pirate.fruit.name+"' and level='"+str(pirate.level)+"' and qualite='"+str(pirate.qualite)+"';"
		InteractBDD.connectAndExecuteRequest(request, True, conn, cur)
